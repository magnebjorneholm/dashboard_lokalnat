"""
core/data_loader_intaktsram.py

Intäktsram-specifik datainläsning och scenariointegrering.

Laddar baseline-data från Excel och integrerar scenarier från:
- Effektiviseringskrav (DEA)
- Kapitalbas (CAPEX-scenarier)

Används av:
- foretag/view/intaktsram_ny.py (IR-dekomposition)
"""
...
from core.data_loader_base import load_dmu_mapping
...
def load_baseline_data(filepath: str) -> pd.DataFrame:
    ...
    Args:
        filepath: Sökväg till Excel-fil (vanligtvis "Löpande kostnader från SDF 2024-27.xlsx")

    Returns:
        DataFrame med standardiserade kolumnnamn inklusive DMU:
        - REId, DMU, Företag
        - Intaktsram_Total
        - Paverkbara_Kostnader, Opaverkbara_Kostnader
        - Kapitalkostnad_Total (eller Avskrivningar + Avkastning separat)
        - Flexibilitetstjanster, Avbrottsersattning_12_24h
        - Källa_* och Uppdaterad_* metadata-kolumner
    """
    try:
    ...
    # Lägg automatiskt till DMU-mappning
...
def detect_scenario_updates() -> Dict[str, Optional[Dict]]:
    """
    Letar efter scenario-filer från andra sektioner i organisationsbaserade sökvägar.
    Returnerar filnamn och tidsstämplar för bättre debugging.

    Returns:
        Dict med information om tillgängliga scenarier för den inloggade organisationen:
        {
            'effektiviseringskrav': {
                'file': 'scenario/effektiviseringskrav/exports_to_ir/org/ir_effkrav_*.parquet',
                'name': 'ir_effkrav_dea_method_20241204.parquet',
                'created': '2024-12-04 15:30'
            } eller None,
            'kapitalbas': {
                'file': 'scenario/kapitalbas/exports_to_ir/org/ir_kapkost_wacc_*.parquet',
...
def load_scenario_data(scenario_type: str, scenario_file: str, baseline_df: pd.DataFrame) -> pd.DataFrame:
    """
    Laddar och integrerar scenario-data från andra sektioner med DMU-baserad merge.

    För effektiviseringskrav returneras bara baseline - applicering sker i separat funktion.
    För kapitalbas uppdateras Kapitalkostnad_Total direkt.
    ...
    try:
        ...
        if scenario_type == 'effektiviseringskrav':
            # Returnera bara baseline - applicering sker i separat funktion
...
def apply_effektiviseringskrav_to_working_df(
    ...
    """
    Applicerar effektiviseringskrav på working_df med vald metod (OPEX eller TOTEX).
    ...
    """
    from effektivitet.backend.ir_calculations import calculate_ir_paverkbara_export
...
def calculate_intaktsram(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: DataFrame med intäktsram-komponenter

    Returns:
        DataFrame med uppdaterad Intaktsram_Beraknad, Delta_Intaktsram och Delta_Procent
    """
    result_df = df.copy()
