...
Tests:
1. opex_scaling=1.10 → OPEXp +10% user only → DEA unchanged (uses baseline) → RF changes
...
4. opex_override=X → user's OPEXp replaced → DEA unchanged (uses baseline)
...
class TestOpexScalingCascade:
    ...
    Pre-DEA: user's COL_CONTROLLABLE_AVG *= 1.10 → TOTEX recalc for user.
    DEA: Unchanged (always uses baseline historical data).
    Post-DEA: sdf_controllable scaled for user → new RF.
    """

    def test_pipeline_runs_successfully(self, pipeline_opex_scaling):
        assert isinstance(pipeline_opex_scaling, PipelineResult)

    def test_user_opex_increased(self, pipeline_result_886, pipeline_opex_scaling):
        """User's OPEXp should increase by ~10%."""
    ...
    def test_user_totex_increased(self, pipeline_result_886, pipeline_opex_scaling):
    ...
    def test_dea_uses_baseline(self, pipeline_opex_scaling):
        """DEA always uses baseline data — not re-executed for OPEX changes."""
        assert pipeline_opex_scaling.dea.dea_executed is False
        assert pipeline_opex_scaling.dea.dea_method == "baseline"

    def test_efficiency_unchanged(self, pipeline_result_886, pipeline_opex_scaling):
        """Efficiency unchanged because DEA uses baseline data."""
        baseline_eff = pipeline_result_886.extraction.efficiency
        scaled_eff = pipeline_opex_scaling.extraction.efficiency
        assert scaled_eff == pytest.approx(baseline_eff, rel=1e-9)

    def test_eff_req_unchanged(self, pipeline_result_886, pipeline_opex_scaling):
        """Eff req unchanged because DEA uses baseline data."""
        baseline_req = pipeline_result_886.post_dea.user_eff_req_pct
        scaled_req = pipeline_opex_scaling.post_dea.user_eff_req_pct
        assert scaled_req == pytest.approx(baseline_req, rel=1e-9)
    ...
    def test_only_user_scaled(self, pipeline_result_886, pipeline_opex_scaling):
        """Only user's company should be scaled, others unchanged."""
        baseline_df = pipeline_result_886.pre_dea.df_all_companies
        scaled_df = pipeline_opex_scaling.pre_dea.df_all_companies

        # User should be scaled
        user_bl = baseline_df[baseline_df["REId"] == USER_REID]["controllable_cost_average"].iloc[0]
        user_sc = scaled_df[scaled_df["REId"] == USER_REID]["controllable_cost_average"].iloc[0]
        assert user_sc == pytest.approx(user_bl * OPEX_SCALING, rel=1e-6)

        # Others should be unchanged
        others_bl = baseline_df[baseline_df["REId"] != USER_REID].set_index("REId")["controllable_cost_average"]
        others_sc = scaled_df[scaled_df["REId"] != USER_REID].set_index("REId")["controllable_cost_average"]
        pd.testing.assert_series_equal(others_bl, others_sc)
...
class TestOpexOverride:
    ...
    Only user's company changes. DEA unchanged (uses baseline data).
    """
    ...
    def test_dea_uses_baseline(self, pipeline_opex_override):
        """DEA always uses baseline data — not re-executed for OPEX override."""
        assert pipeline_opex_override.dea.dea_executed is False
        assert pipeline_opex_override.dea.dea_method == "baseline"
...
class TestScalingPlusOverride:
    ...
    For user: override value trumps (250000, not baseline*1.10).
    For others: unchanged (scaling only applies to user's company).
    """
    ...
    def test_others_unchanged(self, pipeline_result_886, pipeline_scaling_plus_override):
        """Other companies should be unchanged (scaling only affects user)."""
        baseline_df = pipeline_result_886.pre_dea.df_all_companies
        ...
        pd.testing.assert_series_equal(others_bl, others_co)
