...
def capbase_mini():
    ...
    return pd.read_parquet(dataset_path("capbase_a_mini"))


@pytest.fixture(scope="session")
def capcost_a():
    """Load capcost_a.parquet (pre-aggregated capital costs by category)."""
    return pd.read_parquet(dataset_path("capcost_a"))
...
def controllable_detail_mini():
    ...
    return pd.read_parquet(dataset_path("controllable_a_mini"))


@pytest.fixture(scope="session")
def controllable_meta_mini():
    """Load controllable_meta_mini.parquet (3 test companies)."""
    return pd.read_parquet(dataset_path("controllable_meta_mini"))


@pytest.fixture(scope="session")
def non_controllable_detail_mini():
    """Load non_controllable_a_mini.parquet (3 test companies)."""
    return pd.read_parquet(dataset_path("non_controllable_a_mini"))
