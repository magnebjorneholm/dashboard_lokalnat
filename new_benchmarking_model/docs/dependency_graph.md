...
> Claude-konversationer hit innan de rör koden i `new_benchmarking_model/`.
...
- **Entrypoint:** `run_new_benchmarking(cfg)` i `new_benchmarking_model/model.py:87`
...
### Add-on-paketet `new_benchmarking_model/`
...
| Fil | Vad |
|-----|-----|
| `new_benchmarking_model/data/precompute.py` | Förberäknar huvudspecet offline → `new_benchmarking_model/data/precomputed/`. |
| `new_benchmarking_model/data/loader.py` | `load_precomputed_main()` — rekonstruerar `NewBenchmarkingResult` vid körning. |
| `pages/5_new_benchmarking.py` | Streamlit-sidan. Tung DEA fyras bara på knappklick. |
| `new_benchmarking_model/ui/config_panel.py` | `render_config_panel()` — experimentpanelen. |
| `new_benchmarking_model/ui/company_view.py` | Resultatvyn (Sektoröversikt + Ditt företag + TOTEX-brygga). |
...
| Ledningar | `new_benchmarking_model/components/cable_length/` → `load_cable_components()` | km per ledning (för DEA-output ledningslängd). |
...
**Fil:** `new_benchmarking_model/totex/capex_environment.py`
...
**Fil:** `new_benchmarking_model/efficiency/efficiency_requirement_two_sided.py`
...
**Fil:** `new_benchmarking_model/efficiency/cost_impact.py`. Procenten omvandlas till kronor
...
- Förberäknas offline med `new_benchmarking_model/data/precompute.py` → `new_benchmarking_model/data/precomputed/`.
- Laddas vid körning via `new_benchmarking_model/data/loader.py` → `load_precomputed_main()`,
