...
# UPPDATERAD: Stöd för TOTEX-metod vid effektiviseringskrav
...
from intaktsram.app.data_loader import (
    ...
    apply_effektiviseringskrav_to_working_df  # NYTT: För TOTEX-stöd
)
...
# NYTT: Import för effektiviseringskrav-beräkningar
...
def show_foretag_ir_dekomposition():
    ...
    show_component_controls(entity_data, df_company)
...
def show_component_controls(entity_data: pd.Series, df_working: pd.DataFrame):
    ...
    new_paverkbara = st.sidebar.number_input(
        ...
        key="manual_paverkbara_company"
    )
    if st.sidebar.button("Uppdatera manuellt", key="update_paverkbara_company"):
    ...
    if has_detailed_capital:
        ...
        new_avskrivning = st.sidebar.number_input(
            ...
            format="%.6f",  # Behåll decimaler
            key="manual_avskrivning_company"
        )
        new_avkastning = st.sidebar.number_input(
            "Avkastning (tkr)", 
            value=float(current_avkastning),
            format="%.6f",  # Behåll decimaler
            key="manual_avkastning_company"
        )
        
        if st.sidebar.button("Uppdatera komponenter", key="update_detailed_kapital_company"):
        ...
        new_kapital = st.sidebar.number_input(
            ...
            format="%.6f",  # Behåll decimaler
            key="manual_kapital_company"
        )
        if st.sidebar.button("Uppdatera manuellt", key="update_kapital_company"):
...
def update_component_from_scenario(
    ...
    try:
        ...
        if component == 'kapitalkostnad' and source == 'kapitalbas':
            ...
            st.sidebar.success(f"Effektiviseringskrav laddad med {method}-metod för ditt företag")
...
# Logga ut
st.sidebar.markdown("---")
if st.button("Logga ut", key="logout_ir_company"):
