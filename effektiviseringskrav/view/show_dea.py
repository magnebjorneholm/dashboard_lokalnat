...
# Import av geografiska funktioner
from effektiviseringskrav.app.heatmap_utils import (
    load_shapes_for_dea,
...
def calculate_ir_paverkbara_export_fixed(dea_result: pd.DataFrame, ir_baseline_file: str) -> Optional[pd.DataFrame]:
    ...
    for col in available_cols:
        if any(variant in col.lower() for variant in ['företag', 'foretag', 'företag', 'föÂ¶retag']):
            foretag_col = col
            break
...
def show_dea_view(df):
    ...
    if 'dea_result' in st.session_state:
        ...
        try:
            # Ladda shapefile (filtrerat enligt reconciliation)
            ...
            if st.button("Kör grannskapsanalys", type="primary"):
                with st.spinner("Beräknar grannskapseffektivitet..."):
                    try:
                        ...
                        with col1:
                            avg_gap = df_spatial["eff_gap"].mean()
                            st.metric("Medel effektivitetsgap", f"{avg_gap:.3f}")
                        with col2:
            ...
            if Path(ir_baseline_file).exists():
                try:
                    ...
                    if export_data is not None and not export_data.empty:
                        ...
                        with col3:
                            total_reduction = export_data['Total_Reduction_tkr'].sum() / 1000
