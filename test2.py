"""
Intäktsramsvisualisering - Enligt ny Ei-struktur
===============================================
Optimal balans mellan kompakthet och läsbarhet
"""
...
def create_ei_structure_html(data: Dict[str, float]) -> str:
    ...
    <head>
        ...
        <script>
            // Definiera kopplingar mellan boxar och linjer
            const connections = {
                'box-paverkbara': {
...
def main():
    st.set_page_config(page_title="Intäktsramsvisualisering", layout="wide")
    
    st.title("Intäktsramsberäkning")
    st.markdown("Interaktiv visualisering av intäktsramskomponenter enligt Ei:s struktur")
    ...
    html_content = create_ei_structure_html(data)
    ...
    # Detaljerad tabell under visualiseringen
    st.subheader("Komponentdetaljer")
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.markdown("### Löpande kostnader")
        import pandas as pd
        lopande_data = pd.DataFrame({
            'Komponent': ['Påverkbara kostnader', 'Ej påverkbara kostnader', 'Effektiviseringskrav'],
            'Belopp (tkr)': [f"{data['paverkbara']:,}".replace(",", " "), 
                            f"{data['ej_paverkbara']:,}".replace(",", " "),
                            f"-{data['effektivisering']:,}".replace(",", " ")],
            'Andel av total': ['17.6%', '32.0%', '-0.4%']
        })
        st.dataframe(lopande_data, use_container_width=True, hide_index=True)
        
        lopande_total = data['paverkbara'] + data['ej_paverkbara'] - data['effektivisering']
        st.metric(label="Total löpande kostnader", value=f"{lopande_total:,} tkr".replace(",", " "))
    
    with col2:
        st.markdown("### Kapitalkostnader")
        kapital_data = pd.DataFrame({
            'Komponent': ['Avskrivningar', 'Avkastning (WACC)', 'Kvalitetsjustering'],
            'Belopp (tkr)': [f"{data['avskrivningar']:,}".replace(",", " "),
                            f"{data['avkastning']:,}".replace(",", " "),
                            f"{data['kvalitet']:,}".replace(",", " ")],
            'Andel av total': ['29.3%', '19.5%', '2.0%']
        })
        st.dataframe(kapital_data, use_container_width=True, hide_index=True)
        
        kapital_total = data['avskrivningar'] + data['avkastning'] + data['kvalitet']
        st.metric(label="Total kapitalkostnader", value=f"{kapital_total:,} tkr".replace(",", " "))
    
    st.markdown("---")
    
    # Scenariohantering
    st.subheader("Scenariohantering")
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        if st.button("📊 Exportera scenario", use_container_width=True):
            st.success("Scenario exporterat till 'scenario_2024_baseline.parquet'")
    
    with col2:
        if st.button("🔄 Jämför med föregående period", use_container_width=True):
            st.info("Jämförelse med 2020-2023 laddas...")
    
    with col3:
        if st.button("📈 Visa trendanalys", use_container_width=True):
            st.info("Trendanalys för intäktsram 2012-2024 genereras...")
    ...
    intaktsram_total = lopande_total + kapital_total
    
    with col1:
        st.metric(
            label="Total intäktsram",
            value=f"{intaktsram_total:,} tkr".replace(",", " "),
            delta="2.3%"
        )
    
    with col2:
        st.metric(
            label="Effektiviseringskrav",
            value=f"-{data['effektivisering']:,} tkr".replace(",", " "),
            delta="-0.4%",
            delta_color="inverse"
        )
    ...
    with col4:
        st.metric(
            label="Kapitalbas",
            value=f"{data['kapitalbas']:,} tkr".replace(",", " "),
            delta="1.8%"
        )
