...
def get_period_df(df: pd.DataFrame, years=(2024, 2025, 2026, 2027)) -> pd.DataFrame:
    codes = []
    for y in years:
        codes.extend(YEAR_TO_CODES[y])
...
def _read_reconciliation(path_csv: str) -> Optional[pd.DataFrame]:
    try:
        ...
        # Endast relevanta kolumner och rena dtypes
        keep = [c for c in ["id_network","DMU","Företag","REId"] if c in rec.columns]
        rec = rec[keep].drop_duplicates()
...
def _aggregate_to_dmu(df_facit: pd.DataFrame) -> pd.DataFrame:
    """Aggregerar från id_network till DMU-nivå. Filtrerar bort regionnät (RER/RET) via REId."""
    ...
    # Filtrera till lokalnät (REId som börjar på REL)
    ...
    st.session_state['dmu_debug'] = {
        'original_count': df_facit['id_network'].nunique(),
        'reconciliation_count': rec['id_network'].nunique(),
    ...
    if missing_dmu.sum() > 0:
        st.warning(f"{len(unmapped_networks)} id_network saknar DMU-mappning och exkluderas")
        df_with_dmu = df_with_dmu.dropna(subset=['DMU'])

    if df_with_dmu.empty:
        st.error("Ingen data kvar efter DMU-mappning/REL-filter")
        return pd.DataFrame()

    # Aggregera till DMU per halvår (behåll 'time' för års­summa H1+H2 i UI)
...
def _write_ir_export(df_export: pd.DataFrame, tag: str) -> tuple[str,str]:
    ...
    meta = {
        ...
        "constructed_as": (
            ...
            "Endast lokalnät (REId börjar på REL)."
        ),
        "components": {
            "Avskrivningar_Ny": "dep_ord + dep_tail (oförändrat av WACC)",
...
def show_capcost(df_facit: pd.DataFrame) -> None:
    ...
    with TAB3:
        ...
        try:
            ...
            df_ir_export, ir_tag = _build_ir_export_table_period(df, r_new, years=(2024,2025,2026,2027))
