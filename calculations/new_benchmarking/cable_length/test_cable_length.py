...
def test_components_shape_and_columns(components):
    expected = {
        C.COL_ID_FIRM, C.COL_ID_NETWORK, C.COL_SUBCAT,
...
def test_aggregate_one_row_per_firm(components):
    ...
    assert list(res.columns) == [C.COL_ID_FIRM, C.COL_KM_TOTAL]
    assert res[C.COL_ID_FIRM].is_unique
    assert res[C.COL_ID_FIRM].nunique() == components[C.COL_ID_FIRM].nunique()
...
def test_split_by_voltage_reconciles(components):
    ...
    flat = aggregate_cable_length_per_firm(components).set_index(C.COL_ID_FIRM)[C.COL_KM_TOTAL]
    split = aggregate_cable_length_per_firm(components, split_by_voltage=True)

    assert set(split.columns) == {C.COL_ID_FIRM, C.COL_VOLTAGE_LEVEL, C.COL_KM_TOTAL}
    assert split[C.COL_VOLTAGE_LEVEL].isin(C.VOLTAGE_LEVELS).all()

    regrouped = split.groupby(C.COL_ID_FIRM)[C.COL_KM_TOTAL].sum()
