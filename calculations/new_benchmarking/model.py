...
def _build_cable_outputs(cfg: NewBenchmarkingConfig) -> tuple[pd.DataFrame, List[str]]:
    ...
    if not cfg.split_by_voltage:
        ...
        agg = agg.rename(columns={cable_C.COL_ID_FIRM: COL_REID,
                                  cable_C.COL_KM_TOTAL: COL_CABLE_LENGTH_KM})
    ...
    wide = (
        agg.pivot(index=cable_C.COL_ID_FIRM, columns=cable_C.COL_VOLTAGE_LEVEL,
                  values=cable_C.COL_KM_TOTAL)
        .fillna(0.0)
        .reset_index()
        .rename(columns={cable_C.COL_ID_FIRM: COL_REID})
    )
    out_cols = []
